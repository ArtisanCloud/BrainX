...
import {MediaResource} from "@/app/api/media-resource";
...
export async function ActionCreateDocument(option: RequestCreateDocument): Promise<ResponseCreateDocument> {
  ...
  const endpoint = `/api/rag/dataset/document/create`;
...
export async function ActionPatchDocument(option: RequestPatchDocument): Promise<ResponsePatchDocument> {
  ...
  const endpoint = `/api/rag/dataset/document/patch/${option.uuid}`;
...
export async function ActionDeleteDocument(documentUuid: string): Promise<ResponseDeleteDocument> {
  ...
  const endpoint = `/api/rag/dataset/document/delete/${documentUuid}`
...
export interface Segmentation {
  ...
  overlap_chunk_length: number
  replace_consecutive_spaces: boolean
  delete_all_urls: boolean
}
...
export interface RequestAddContent {
  dataset_uuid: string
...
export interface ResponseAddContent{
  data:Document[]
  task_ids:string[]
}

export async function ActionAddContent(option: RequestAddContent): Promise<ResponseAddContent> {

  const endpoint = `/api/rag/dataset/document/add-content`

  const res = await backendClient.backend_post(endpoint, option);

  return res as ResponseAddContent;
