...
import ModelAddModels
  from "@/app/(workspace)/space/settings/model-provider/system-model-setting/to-config-providers/model-add-models";
import ModelSettingProvider
  from "@/app/(workspace)/space/settings/model-provider/system-model-setting/to-config-providers/model-setting-provider";
  ...
  return (
    <div className={styles.container}>
      {Object.entries(providers).map(([key, provider]) => {
        ...
        const providerDescription = provider.description?.zh_CN
        // const providerBackgroundColor = provider.background || "#f5f5f5"; // 默认背景色
        const providerBackgroundColor = "linear-gradient(to right, #e0e0e0, #eee)";
        ...
        const actionButtons = [
          {
            ...
            component: <ModelSettingProvider provider={provider}/>
          },
          {
            key: provider.provider+"-add-models",
            type: "modelSetting",
            component: <ModelAddModels provider={provider}/>
          },
        ];
        ...
        const visibleButtons = actionButtons.filter((button) => {
          if (
            (button.type === "providerSetting" && provider.provider_credential_schema) ||
            (button.type === "modelSetting" && provider.models_credential_schema)
          ) {
        ...
        const gridCols = visibleButtons.length === 1 ? "grid-cols-1" : "grid-cols-2";
        ...
        return (
          ...
          >
            <div className="flex-col">
              ...
              <p className="mt-1 leading-4 text-xs text-black/[48] line-clamp-4">{providerDescription}</p>
            </div>
