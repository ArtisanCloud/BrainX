...
sync_session_local = sessionmaker(sync_db_engine, expire_on_commit=False)
...
def test_sync_connection_pool_auto_shrink(operation):
    ...
    # 模拟多个并发操作，查看池的增长
        ...
        with sync_session_local() as session2:
            for index in range(10):
                ...
                print(
                    f"Checked-out connections after operation {index + 1}: {pool.checkedout()}"
                )
    ...
    # 设置pool_timeout为3秒，等待5秒确保超时后连接会被回收
        ...
        # 断言池大小和活动连接数
        assert (
            pool_size_after_timeout <= initial_pool_size
        ), "Connection pool size should not increase after timeout."
        assert (
            checked_out_after_timeout <= initial_checked_out
        ), "Checked-out connections should decrease after timeout."

    # 断言连接池最后不应该有未归还的连接
    assert (
        pool.checkedout() == 0
    ), "There should be no checked-out connections after all operations."
