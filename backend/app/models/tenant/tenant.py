...
class Tenant(BaseORM):
    ...
    users: Mapped[List["User"]] = relationship(
        secondary="public." + table_name_pivot_tenant_to_user,
        back_populates="tenants",
        overlaps="user, tenant",
    )
    owned_user: Mapped["User"] = relationship(
        back_populates="owned_tenant", foreign_keys="[User.tenant_owner_uuid]"
    )
    apps: Mapped[List["App"]] = relationship(back_populates="tenant")
    model_providers: Mapped[List["ProviderModel"]] = relationship(
        back_populates="tenant"
    )
    datasets: Mapped[List["Dataset"]] = relationship(
        back_populates="tenant", foreign_keys="[Dataset.tenant_uuid]"
    )

    def __repr__(self):
        encrypted_key_display = (
            self.encrypted_public_key[:10] if self.encrypted_public_key else "None"
        )
        config_display = self.config[:10] if self.config else "None"
        ...
        return (
            ...
            f"encrypted_public_key='{encrypted_key_display}...', "
