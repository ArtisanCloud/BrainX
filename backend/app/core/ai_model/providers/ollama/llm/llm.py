...
class OllamaLargeLanguageModel(LargeLanguageModel):
    ...
    def _generate(
        ...
        if format_schema := model_parameters.pop("format", None):
            try:
                data["format"] = (
                    format_schema
                    if format_schema == "json"
                    else json.loads(format_schema)
                )
    ...
    def get_customizable_model_schema(
        ...
        entity = AIModelEntity(
            model=model,
            label=I18nObject(zh_CN=model, en_US=model),
            ...
            parameter_rules=[
                ...
                ParameterRule(
                    ...
                    label=I18nObject(en_US="Top P", zh_CN="Top P"),
                    type=ParameterType.FLOAT,
                    help=I18nObject(
                        ...
                        zh_CN="与top-k一起工作。较高的值（例如，0.95）会导致生成更多样化的文本，而较低的值（例如，0.5）会生成更专注和保守的文本。（默认值：0.9）",
                    ),
                    default=0.9,
                    min=0,
                    max=1,
                ...
                ParameterRule(
                    name="top_k",
                    label=I18nObject(en_US="Top K", zh_CN="Top K"),
                    type=ParameterType.INT,
                    help=I18nObject(
                        en_US="Reduces the probability of generating nonsense. "
                        "A higher value (e.g. 100) will give more diverse answers, "
                        "while a lower value (e.g. 10) will be more conservative. (Default: 40)",
                        zh_CN="减少生成无意义内容的可能性。较高的值（例如100）将提供更多样化的答案，而较低的值（例如10）将更为保守。（默认值：40）",
                    ),
                    min=1,
                    max=100,
                ),
                ParameterRule(
                    ...
                    help=I18nObject(
                        ...
                        zh_CN="设置对重复内容的惩罚强度。一个较高的值（例如，1.5）会更强地惩罚重复内容，而一个较低的值（例如，0.9）则会相对宽容。（默认值：1.1）",
                    ),
                    min=-2,
                    max=2,
                ),
                ParameterRule(
                    ...
                    label=I18nObject(en_US="Num Predict", zh_CN="最大令牌数预测"),
                    type=ParameterType.INT,
                    help=I18nObject(
                        en_US="Maximum number of tokens to predict when generating text. "
                        "(Default: 128, -1 = infinite generation, -2 = fill context)",
                        zh_CN="生成文本时预测的最大令牌数。（默认值：128，-1 = 无限生成，-2 = 填充上下文）",
                    ),
                    default=(
                ...
                ParameterRule(
                    name="mirostat",
                    label=I18nObject(en_US="Mirostat sampling", zh_CN="Mirostat 采样"),
                    type=ParameterType.INT,
                    help=I18nObject(
                        en_US="Enable Mirostat sampling for controlling perplexity. "
                        "(default: 0, 0 = disabled, 1 = Mirostat, 2 = Mirostat 2.0)",
                        zh_CN="启用 Mirostat 采样以控制困惑度。"
                ...
                ParameterRule(
                    name="mirostat_eta",
                    label=I18nObject(en_US="Mirostat Eta", zh_CN="学习率"),
                    type=ParameterType.FLOAT,
                    help=I18nObject(
                        ...
                        "(Default: 0.1)",
                        zh_CN="影响算法对生成文本反馈响应的速度。较低的学习率会导致调整速度变慢，而较高的学习率会使得算法更加灵敏。（默认值：0.1）",
                    ),
                    precision=1,
                ),
                ParameterRule(
                    name="mirostat_tau",
                    label=I18nObject(en_US="Mirostat Tau", zh_CN="文本连贯度"),
                    type=ParameterType.FLOAT,
                    help=I18nObject(
                        en_US="Controls the balance between coherence and diversity of the output. "
                        "A lower value will result in more focused and coherent text. (Default: 5.0)",
                        zh_CN="控制输出的连贯性和多样性之间的平衡。较低的值会导致更专注和连贯的文本。（默认值：5.0）",
                    ),
                    precision=1,
                ),
                ParameterRule(
                ...
                ParameterRule(
                    name="num_gpu",
                    label=I18nObject(en_US="GPU Layers", zh_CN="GPU 层数"),
                    type=ParameterType.INT,
                    help=I18nObject(
                        ...
                        zh_CN="加载到 GPU 的层数。在 macOS 上，默认为 1 以启用 Metal 支持，设置为 0 则禁用。"
                ...
                ParameterRule(
                    name="num_thread",
                    label=I18nObject(en_US="Num Thread", zh_CN="线程数"),
                    type=ParameterType.INT,
                    help=I18nObject(
                        ...
                        zh_CN="设置计算过程中使用的线程数。默认情况下，Ollama会检测以获得最佳性能。建议将此值设置为系统拥有的物理CPU核心数（而不是逻辑核心数）。",
                    ),
                    min=1,
                ),
                ParameterRule(
                    name="repeat_last_n",
                    label=I18nObject(en_US="Repeat last N", zh_CN="回溯内容"),
                    type=ParameterType.INT,
                    help=I18nObject(
                        en_US="Sets how far back for the model to look back to prevent repetition. "
                        "(Default: 64, 0 = disabled, -1 = num_ctx)",
                        zh_CN="设置模型回溯多远的内容以防止重复。（默认值：64，0 = 禁用，-1 = num_ctx）",
                    ),
                    min=-1,
                ),
                ParameterRule(
                    name="tfs_z",
                    label=I18nObject(en_US="TFS Z", zh_CN="减少标记影响"),
                    type=ParameterType.FLOAT,
                    help=I18nObject(
                        en_US="Tail free sampling is used to reduce the impact of less probable tokens "
                        "from the output. A higher value (e.g., 2.0) will reduce the impact more, "
                        "while a value of 1.0 disables this setting. (default: 1)",
                        zh_CN="用于减少输出中不太可能的标记的影响。较高的值（例如，2.0）会更多地减少这种影响，而1.0的值则会禁用此设置。（默认值：1）",
                    ),
                    precision=1,
                ),
                ParameterRule(
                    name="seed",
                    label=I18nObject(en_US="Seed", zh_CN="随机数种子"),
                    type=ParameterType.INT,
                    help=I18nObject(
                        ...
                        zh_CN="设置用于生成的随机数种子。将此设置为特定数字将使模型对相同的提示生成相同的文本。（默认值：0）",
                    ),
                ),
                ParameterRule(
                    name="keep_alive",
                    label=I18nObject(en_US="Keep Alive", zh_CN="模型存活时间"),
                ...
                ParameterRule(
                    name="format",
                    label=I18nObject(en_US="Format", zh_CN="返回格式"),
                    type=ParameterType.TEXT,
                    default="json",
                    help=I18nObject(
                        en_US="the format to return a response in. Format can be `json` or a JSON schema.",
                        zh_CN="返回响应的格式。目前接受的值是字符串`json`或JSON schema.",
                    ),
                ),
            ],
            pricing=PriceConfig(
