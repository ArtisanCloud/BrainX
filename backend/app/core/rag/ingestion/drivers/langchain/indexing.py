...
class LangchainIndexer(BaseIndexing):
    """
       Implementation of BaseIndexing for Langchain ingestion.
       """
    ...
    def save_nodes_to_store_vector(self, nodes: List[DocumentNode]) -> Tuple[int, int, Optional[Exception]]:
        ...
        try:
            ...
            for node, vector in zip(nodes, result_list):
                # print("node:", node.page_content)
                # print("vector:", vector)
                word_count += len(node.page_content)
                # word_count += len(vector.page_content.split())
