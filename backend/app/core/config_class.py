...
from pydantic import ConfigDict, AnyHttpUrl, EmailStr, field_validator

from dotenv import load_dotenv
# 加载环境变量
load_dotenv()

class AppConfig(ConfigDict):
    """
...
class PreviewPrefixedSettings(ConfigDict):
    """
...
class Settings(PreviewPrefixedSettings):
    ...
    def MINIO_ENDPOINT_URL(self) -> str:
        """
        Used for setting Minio endpoint URL in the Minio module.
        When running locally, this should be set to the localstack endpoint.
        """
        return None if self.RENDER else "http://localhost:9001"
