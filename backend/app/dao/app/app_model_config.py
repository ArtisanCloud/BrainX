...
class AppModelConfigDAO(BaseDAO[AppModelConfig]):
    def __init__(self, async_db: AsyncSession = None, sync_db: Session = None):
        super().__init__(AppModelConfig, async_db, sync_db)
    ...
    async def create_app_model_config(self, config_data: AppModelConfigSchema) -> Tuple[
        ...
        try:
            ...
            self.async_db.add(app_model_config)
    ...
    async def get_app_model_config_by_id(self, config_id: int) -> Tuple[
        AppModelConfig | None, SQLAlchemyError | None]:
        try:
            app_model_config = await self.async_db.execute(
    ...
    async def delete_app_model_config(self, config_id: int) -> Tuple[bool, SQLAlchemyError | None]:
        try:
            ...
            await self.async_db.delete(app_model_config)
